...
**Testing and validation.** [Digital Twin](patterns/digital-twin.md) clones dependencies for deterministic testing, while [Validation Constraint](patterns/validation-constraint.md) validates agent output through observable behaviour rather than code review. TDD in [Specify Plan Ship](patterns/specify-plan-ship.md) naturally produces the test suite that Validation Constraint relies on.
